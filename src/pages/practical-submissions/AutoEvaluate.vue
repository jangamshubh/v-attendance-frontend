        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="field p-fluid">
                    <label for="submission_marks_before_deadline1">Marks for Submissions before Deadline / {{ practical.total_marks }} </label>
                ...
                <div class="field p-fluid">
                    <label for="submission_marks_after_deadline1">Marks for Submissions after Deadline / {{ practical.total_marks }} </label>
...
export default {
    ...
    methods: {
        ...
        redirectAfterSuccess() {
            setTimeout(() => this.$router.push({ name: 'practical-submissions.index', params: { practical_id: this.$route.params.practical_id } }), 3000)
        },
        getAssignment() {
            axios.get(`${process.env.VUE_APP_API_URL}/practicals/${this.$route.params.practical_id}/show`, { headers: authHeader() }).then(data => {
                // console.log(data.data)
                let response = data.data;
                if(response.status == 'Success') {
                    this.practical = response.practical;
        ...
        saveMarks() {
            axios.post(`${process.env.VUE_APP_API_URL}/practicals/${this.$route.params.practical_id}/submissions/auto-evaluate`,this.marks,{ headers: authHeader() }).then(data => {
                let response = data.data;
                if(response.status == 'Success') {
                    this.showSuccess();
