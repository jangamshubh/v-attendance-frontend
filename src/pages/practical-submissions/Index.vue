        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header flex justify-content-between flex-column sm:flex-row">
                    <h5>Submissions</h5>
                    <Button type="button" v-if="this.checkRole == 'Teacher'" icon="pi pi-arrow-left" label="Practicals" class="p-button-outlined mb-2" @click="redirectToAssignmentsPage()"/>
                ...
                :globalFilterFields="['']" >
                    ...
                    <Column field="student_name" header="Student" style="min-width:12rem">
                        <template #body="{data}">
                            {{ data.practical_submission_get_students.name }} - {{ data.practical_submission_get_students.roll_no }}
                        </template>
                    ...
                    <Column field="obtained_marks" header="Obtained Marks" style="min-width:12rem">
                        <template #body="{data}">
                            <b v-if="data.obtained_marks != null">{{ data.obtained_marks }} / {{ data.practical_submission_get_practicals.total_marks}}</b>
...
export default {
    ...
    methods: {
        ...
        getAllSubmissions() {
            axios.get(`${process.env.VUE_APP_API_URL}/practicals/${this.$route.params.practical_id}/submissions`, { headers: authHeader() }).then(data => {
        ...
        addMarks(data) {
            this.$router.push({ name: 'practical-submissions.add-marks', params: { practical_id: this.$route.params.practical_id, id: data.id }});
        },
        showSubmission(data) {
            this.$router.push({ name: 'practical-submissions.show', params: { practical_id: this.$route.params.practical_id , id: data.id }});
        },
        redirectToAssignmentsPage() {
            this.$router.push({ name: 'practicals.index' });
        },
        redirectToCreatePage() {
            this.$router.push({ name: 'practical-submissions.create'});
        },
        autoEvaluate() {
            this.$router.push({ name: 'practical-submissions.auto-evaluate', params: { practical_id: this.$route.params.practical_id }});
        },
    },
